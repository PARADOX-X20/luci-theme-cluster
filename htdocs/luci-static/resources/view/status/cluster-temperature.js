/**
 * cluster - Temperature Status View
...
function t(key) {
  if (typeof window !== "undefined" && typeof window.clusterT === "function")
...
return view.extend({
  ...
  buildTooltipRow: function (label, value, color) {
    ...
    if (color)
      keyNodes.push(
        E("span", {
          class: "cluster-temp-tooltip-swatch",
          style: "--sensor-color:%s".format(color),
        }),
      );

    keyNodes.push(E("span", { class: "cluster-temp-tooltip-label" }, [label]));

    return E("div", { class: "cluster-temp-tooltip-row" }, [
      E("div", { class: "cluster-temp-tooltip-key" }, keyNodes),
      E("strong", { class: "cluster-temp-tooltip-value" }, [value]),
    ]);
  },
  ...
  showChartTooltip: function (state) {
    ...
    if (state.title)
      headNodes.push(
        E("strong", { class: "cluster-temp-tooltip-title" }, [state.title]),
      );

    if (state.time)
      headNodes.push(
        E("span", { class: "cluster-temp-tooltip-time" }, [state.time]),
      );

    tooltip.replaceChildren(
      E("div", { class: "cluster-temp-tooltip-head" }, headNodes),
      E("div", { class: "cluster-temp-tooltip-body" }, bodyNodes),
    );
  ...
  buildLayout: function () {
    this.windowSelectorNode = E("div", {
      class: "cluster-temp-window-selector",
    });
    this.buildWindowSelector();

    this.selectorNode = E("div", { class: "cluster-temp-selector" });
    this.chartTitleNode = E("h3", { class: "cluster-temp-chart-title" }, [
      t("Temperature history"),
    ]);
    this.chartToolbarNode = E("div", { class: "cluster-temp-chart-toolbar" }, [
      E("span", { class: "cluster-temp-chart-toolbar-label" }, [t("Window")]),
      this.windowSelectorNode,
    ]);
    this.chartSummaryNode = E("div", { class: "cluster-temp-chart-summary" });
    this.svgNode = createSvgElement("svg", {
      class: "cluster-temp-chart-svg",
      viewBox: "0 0 %d %d".format(GRAPH_WIDTH, GRAPH_HEIGHT),
      preserveAspectRatio: "none",
      role: "img",
      "aria-label": t("Temperature history chart"),
    });
    this.emptyNode = E("div", { class: "cluster-temp-chart-empty" }, [
      t("No temperature sensors found"),
    ]);
    this.chartTooltipNode = E("div", {
      class: "cluster-temp-chart-tooltip",
      hidden: "hidden",
    });
    this.chartWrapNode = E("div", { class: "cluster-temp-chart-wrap" }, [
      this.svgNode,
      this.emptyNode,
      this.chartTooltipNode,
    ]);
    this.statusNode = E("div", { class: "cluster-temp-realtime-status" });
    this.tableBody = E("tbody");

    return E("div", { class: "cbi-map cluster-temp-realtime-page" }, [
      ...
      E("div", { class: "cbi-section cluster-temp-chart-section" }, [
        E("div", { class: "cluster-temp-chart-head" }, [
          E("div", { class: "cluster-temp-chart-title-wrap" }, [
            this.chartTitleNode,
          ]),
      ...
      E("div", { class: "cbi-section cluster-temp-table-section" }, [
        E("h3", { class: "cluster-temp-table-title" }, [t("Sensor statistics")]),
        E("div", { class: "table cluster-temp-table" }, [
          E("table", {}, [
            E("thead", {}, [
  ...
  buildSelector: function (sensors) {
    ...
    this.selectorNode.appendChild(
      E(
        "button",
        {
          class: "cluster-temp-selector-chip cluster-temp-selector-chip-all",
          type: "button",
          "data-active": this.selectedSensorId == null ? "true" : "false",
          style: "--sensor-color:var(--cluster-muted)",
        ...
        [
          E("span", { class: "cluster-temp-selector-label" }, [
            t("All sensors"),
          ]),
        ],
      ),
    ...
    sensors.forEach(
      L.bind(function (sensor) {
        const button = E(
          "button",
          {
            class: "cluster-temp-selector-chip",
            type: "button",
            "data-active":
          ...
          [
            E("span", { class: "cluster-temp-selector-label" }, [sensor.label]),
            E("span", { class: "cluster-temp-selector-meta" }, [
              formatTemp(sensor.temp, 1),
            ]),
          ],
        );
  ...
  buildSummaryCard: function (label, value, extraClass) {
    return E(
      ...
      {
        class:
          "cluster-temp-summary-card" + (extraClass ? " " + extraClass : ""),
      },
      [
        E("span", { class: "cluster-temp-summary-label" }, [label]),
        E("strong", { class: "cluster-temp-summary-value" }, [value]),
      ],
    );
  },
  ...
  createAxisLabel: function (text, x, y, anchor, cssClass) {
    return createSvgElement(
      "text",
      {
        x: String(x),
        y: String(y),
        class: cssClass || "cluster-temp-axis-label",
  ...
  renderChart: function (sensor) {
      ...
      L.bind(function (line) {
        ...
        svg.appendChild(
          createSvgElement("line", {
            ...
            y2: String(y),
            class:
              "cluster-temp-threshold cluster-temp-threshold-" + line.cssClass,
          }),
        );
        svg.appendChild(
          this.createAxisLabel(
        ...
        if (sensor) {
          ...
          svg.appendChild(
            createSvgElement("line", {
              ...
              x2: String(GRAPH_WIDTH - GRAPH_PADDING.right),
              y2: String(averageY),
              class: "cluster-temp-average-line",
            }),
          );
        }
    ...
    const hoverGuide = createSvgElement("line", {
      ...
      class: "cluster-temp-hover-guide",
    });
    const hoverPoints = createSvgElement("g", {
      class: "cluster-temp-hover-points",
    });
    const hoverLayer = createSvgElement("rect", {
      ...
      width: String(plotWidth),
      height: String(plotHeight),
      class: "cluster-temp-hover-layer",
    });
      ...
      dots.forEach(function (dot) {
        hoverPoints.appendChild(
          createSvgElement("circle", {
            ...
            cy: String(dot.y),
            r: "4.5",
            class: "cluster-temp-hover-point",
      ...
      this.tableBody.appendChild(
        E("tr", { class: "tr" }, [
          E(
            "td",
            {
              class: "td cluster-temp-table-empty",
              colspan: "6",
            },
            [t("No temperature sensors found")],
          ),
    ...
    sensors.forEach(
      L.bind(function (sensor) {
        ...
        const row = E("tr", { class: "tr" }, [
          E("td", { class: "td cluster-temp-sensor-cell" }, [
            E(
              "span",
              {
                class: "cluster-temp-sensor-chip",
                style: "--sensor-color:%s".format(this.getColor(sensor.id)),
              },
              [sensor.label],
            ),
            E(
              "div",
              {
                class: "cluster-temp-sensor-path",
          ...
          E(
            "td",
            {
              class: "td cluster-temp-metric-value",
          ...
          E(
            "td",
            { class: "td cluster-temp-metric-value", "data-label": t("Peak") },
            [formatTemp(peak, 1)],
          ),
          E(
            "td",
            { class: "td cluster-temp-status-cell", "data-label": t("Status") },
            [
              E(
                "span",
                {
                  class: "cluster-temp-state-badge",
                  "data-level": sensor.level,
                },
                [getTempStatus(sensor.level)],
              ),
