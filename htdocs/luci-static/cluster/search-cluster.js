/**
 * cluster - Search UI and Indexing
...
(function () {
  ...
  const LIVE_TARGET_PREFIX = "cluster-search-live-";
  const MIN_FUZZY_TOKEN_LENGTH = 4;
  const PREFETCH_FRAME_NAME = "cluster-search-prefetch-frame";
  ...
  const HASH_HIGHLIGHT_CLASS = "cluster-anchor-highlight-target";
  const SEARCH_CACHE_VERSION = 2;
  const LEGACY_SEARCH_CACHE_STORAGE_KEY = "cluster-search-prefetch-cache";
  const LEGACY_SEARCH_CACHE_META_KEY = "cluster-search-prefetch-cache-meta";
  const SEARCH_CACHE_OBJECT = "luci.cluster-search-cache";
  ...
  const SEARCH_INDEX_SETTINGS_HASH = "cluster-search-index-panel";
  const SEARCH_INDEX_SETTINGS_FOCUS_EVENT = "cluster-focus-search-index-panel";
  ...
  function dispatchSearchIndexState() {
    ...
    window.dispatchEvent(
      new CustomEvent("cluster-search-index-state", { detail: detail }),
    );
  }
  ...
  async function callSearchCacheRpc(method, payload) {
    ...
    try {
      ...
      if (error && error.name === "AbortError") {
        console.warn("[cluster] Search cache RPC timed out:", method);
      }
  ...
  function translate(text) {
    if (typeof window.clusterT === "function") {
      try {
        const translated = window.clusterT(text);
        if (translated && translated !== text) {
          return translated;
        }
      } catch (e) {
        // Игнорируем ошибки clusterTranslations
      }
    }
  ...
  function getSemanticRegistry() {
    if (
      window.clusterSearchData &&
      Array.isArray(window.clusterSearchData.semanticEntries)
    ) {
      return window.clusterSearchData.semanticEntries;
    }
    ...
    preindexLoading = (async () => {
      ...
      for (let index = 0; index < routes.length; index++) {
        ...
        try {
          ...
          console.warn("[cluster] Failed to preindex route:", route, error);
        } finally {
          dispatchSearchIndexState();
        }
      ...
      if (shouldReplaceCache) {
        ...
        if (!(await savePrefetchCache(routeHash, completedAt))) {
          ...
          console.warn("[cluster] Failed to persist search prefetch cache.");
        }
        indexMenu(true);
        ...
        [
          {
            ...
            boxShadow: "0 0 0 0 rgba(175,175,175,0)",
            outlineColor: "rgba(175,175,175,0)",
          },
          {
            backgroundColor: "rgba(175,175,175,0.16)",
            boxShadow: "0 0 0 8px rgba(175,175,175,0.14)",
            outlineColor: "rgba(175,175,175,0.92)",
            offset: 0.18,
          },
          {
            backgroundColor: "rgba(175,175,175,0)",
  ...
  async function ensureLiveDataLoaded() {
    ...
    liveDataLoading = Promise.allSettled([
      ...
      .catch((error) => {
        console.warn("[cluster] Failed to load live search data:", error);
        return [];
      })
      .finally(() => {
  ...
  function renderNoResults() {
    ...
    noMatch.className = "cluster-search-empty";

    const noMatchText = document.createElement("div");
    noMatchText.className = "cluster-search-empty-text";
    ...
    noMatchActionLabel.className = "cluster-search-empty-prompt";
    ...
    noMatchAction.className = "cluster-search-empty-action";
  ...
  function renderResults(matches, queryVariants) {
    ...
    for (let i = 0; i < maxResults; i++) {
      ...
      link.className = "cluster-search-result-item";
      ...
      const metaHtml = metaText
        ? `<span class="cluster-search-result-meta">${escapeHtml(metaText)}</span>`
        : "";

      link.innerHTML = `<span class="cluster-search-result-main"><span class="cluster-search-result-title">${titleHtml}</span></span>${metaHtml}`;

      link.addEventListener("mouseenter", () => {
        const items = searchResults.querySelectorAll(
          "a.cluster-search-result-item",
        );
  ...
  function initSearch() {
    const containerEl = document.getElementById("cluster-search-container");
    if (!containerEl) return;
    searchContainer = containerEl;

    const wrapper = document.createElement("div");
    wrapper.className = "cluster-search-wrapper";

    const iconLabel = document.createElement("label");
    iconLabel.className = "cluster-search-icon";
    iconLabel.innerHTML =
    ...
    searchInput.className = "cluster-search-input";
    searchInput.autocomplete = "off";

    searchResults = document.createElement("div");
    searchResults.className = "cluster-search-results";
    searchResults.id = "cluster-search-results";
    ...
    searchInput.setAttribute("aria-controls", "cluster-search-results");
  ...
  function onKeyDown(e) {
    const items = searchResults.querySelectorAll("a.cluster-search-result-item");
